...
r = EA.validate_path("probe10.esm"); println("validate: is_valid=$(r.is_valid) struct=$(length(r.structural_errors))")
...
provT = EarthSciIO.discrete_provider(cache, "$BASE.I3.4x5.nc", [10800.0*k for k in 0:7];
        format="netcdf", variables=["T"], time_dim="time", records_per_sample=2)
...
prob = EA.esm_problem(EA.load_path("probe10.esm"), (T0, T0+1.0);
        providers=Dict("GEOSFP.PS"=>prov, "GEOSFP.T"=>provT),
