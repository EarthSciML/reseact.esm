...
runner and `../../run_reseact.jl` actually depend on. Which compiled program they
get is decided one level up, in `../rx_rhs.jl`. The probe and experiment
...
1. **Compiling needs the out-of-place build.** An in-place `f!` captures a host
   scratch buffer per `_VecNode`, so it is *fundamentally* not compilable — the
   extension's docstring says so. Both runners build `form = :oop`, per split part.
   That build product is the compiled tree-walk IR, not a callable: the emitter
   lowers it, and calling it on host arrays raises
   `E_TREEWALK_OOP_NOT_EVALUABLE`.

2. **Live forcing survives compilation.** The 3-arg wrapper **refuses** a program
   over host buffers (audit J5); the supported route is
   `direct_rhs_with_buffers(fo)` compiled as `(u,p,t,dev)` with
   ...
   observed on the next call with **no recompile** — which is what makes the
   macro-stepped runner possible at all.

3. **The compiled program is grid-independent, but only after several emitter
   fixes.** The original fear — that the pointwise chemistry lift degraded to
   O(#cells) scalar-spine reads — was real, and it was fixed upstream in EarthSciAST
   rather than worked around here: lane-batching the per-cell scalar surface, a
   level-major prefix scan, and one read per distinct (value, window). Together those
   took the CONUS compile from *never finished in 3h45m* to ~550 s.
   [../../COMPILE_COST.md](../../COMPILE_COST.md) carries the same story for the
   direct emitter's own read lowering.
...
| `rx_sym_block_jac.jl` | the adjoint driver, `run_reseact_reactant.jl` | EarthSciASTDiff's analytic band model turned inside out into the two GATHERS a compiled program can execute (`u → uj`, `duj → per-cell blocks`), plus `validate_plan`, which checks both of them against the evaluator's own scatter map at one point. |
...
- Buffers/refresh API: `.../src/tree_walk/oop.jl` (`forcing_buffers`),
  `.../ext/reactant_direct/api.jl` (`direct_rhs` / `direct_rhs_with_buffers`) and
  `.../src/data_refresh.jl` (`sync_forcing!`).
- Devices and shardings: `.../docs/src/compiled-backend-devices.md`.
