...
# already have: `CapacityChem`, `RxTracedIntegrator`, `RxSymBlockJac`, `RxRHS`, and
# the split_common.jl top-levels `stencil_following_rule` /
# `index_promoted_refs_by_loop!`.
#
# EVERY COMPILED PROGRAM HERE IS BUILT THROUGH `RxRHS` (tools/rx_rhs.jl), the
# one seam this repository emits right-hand sides through: the capacity RHS,
# the symbolic Jacobian's band model, and the host evaluations the two build
# checks below run on. A shard that resolved a different EarthSciAST than the
# driver would compile, run and answer, and its answer would look like the
# driver's -- so `build!` prints the lane banner on the shard's own first line.
# ===========================================================================
module ShardKernel
...
for _n in (:CapacityChem, :RxTracedIntegrator, :RxSymBlockJac, :RxRHS,
...
function build!(st::ShardState, cfg::Dict, ca::Dict{String,Any},
    ...
    C = st.C; NS = st.NS
    # the shard's own first line says what it compiled through: a shard that
    # resolved a different EarthSciAST than the driver would compile, run and
    # answer, and the answer would look like the driver's.
    wsay(st, RXR.rx_rhs_banner())
...
function refresh!(st::ShardState, lanes::Dict{String,Any})
    ...
    RXR.rx_sync!(st.dev_bufs, st.f)
    RXR.rx_sync!(st.dev_bufsJ, st.jacE.fJ!)
    return nothing
end
