...
#
# RE-GATED ON THE DIRECT StableHLO EMITTER, 2026-09-15, same configuration at
# N=0 and N=2 (the `inprocess` executor, i.e. the seam's other implementation):
# the ladder is the SAME 3/0,116/3 2/1,92/2 3/0,30/0 as the 2026-09-05 record,
# J = 39.68329300374256 unsharded against 39.68329300374257 sharded, the 21
# nonzero components agree to 1.325e-11 worst relative (Transport3D.dlon_deg)
# with no component moving between zero and nonzero, and the structural identity
# holds at 1.724e-16 / 1.551e-15. So the DECOMPOSITION reproduces itself across
# both the emitter change and the executor change, which is the whole claim; the
# one-ulp J and the 1e-11 class are the partial-norm reassociation this header
# already predicts. The `process` executor was not re-gated in-session -- see
# tools/diag/shard_exec_gate.sbatch, which is where these belong.
#
