...
# ---- descriptor census: which acc-descriptor KINDS carry the residual reads --
# (ess-oop-ssa ghost extension) A per-kind tally over every vectorizable kernel's
# TOP-LEVEL descriptors, plus the ghost-bearing subset, so the blocker tally
# above can be traced to a concrete read surface.
const KN = Dict(EA._AK_STATE_AFFINE => :state_affine,
                EA._AK_STATE_TBL_BOX => :state_tbl_box,
                EA._AK_STATE_FIXED => :state_fixed,
                EA._AK_STATE_INDIRECT => :state_indirect,
                EA._AK_STATE_INDIRECT_COL => :state_indirect_col)
function desc_census(label, kernels, plans)
    kinds = Dict{Symbol,Int}(); gh = Dict{Symbol,Int}(); ghlanes = 0; ghdesc = 0
    nsub = 0; nsubdesc = 0; nred = 0; nreddesc = 0
    for (K, pl) in zip(kernels, plans)
        pl.vectorizable || continue
        for i in eachindex(K.acc)
            nm = get(KN, K.acc[i].kind, Symbol("k", Int(K.acc[i].kind)))
            kinds[nm] = get(kinds, nm, 0) + 1
            if !isempty(pl.ghost[i])
                gh[nm] = get(gh, nm, 0) + 1
                ghdesc += 1
                ghlanes += count(pl.ghost[i])
            end
        end
        nsub += length(pl.subs)
        for sp in pl.sub_plans; nsubdesc += count(g -> !isempty(g), sp.gathers); end
        nred += length(pl.red_plan)
        for ep in pl.red_plan; nreddesc += count(g -> !isempty(g), ep.gathers); end
    end
    say("P12_$label kinds=" * repr(sort(collect(kinds), by = first)))
    say("P12_$label ghostkinds=" * repr(sort(collect(gh), by = first)) *
        "  ghost_desc=$ghdesc ghost_lanes=$ghlanes")
    say("P12_$label subs=$nsub subdesc=$nsubdesc reduces=$nred reddesc=$nreddesc")
end
for (li, lv) in enumerate(ml)
    desc_census("L$li", lv[2], lv[3])
end
desc_census("FIN", r.acc_kernels, r.acc_plans)
# per-producer residual reasons, for the producers that did NOT skip
say("P12_PRODLEN " * repr([length(p.out_slots) for lv in ml for p in lv[3]]))
say("P10_DONE")

# ---- the scatter-skip GATE's inputs, per producer (ess-oop-ssa-gate) --------
# `oop_ssa_producers` (EarthSciAST branch perf/ssa-scatter-skip-gate) reports,
# for every tracked fill kernel: its value length, how many redirected read
# surfaces now source from it and at what element volume, how many take the
# whole value with no op, and the static vs gated skip verdicts. This is the
# table the gate's discriminator was chosen from -- print it whole, per part,
# so chemistry and transport producers can be compared directly.
if isdefined(EA, :oop_ssa_producers)
    pr = EA.oop_ssa_producers(fo)
    say(@sprintf("P14_PROD  n=%d skippable=%d skipped=%d", length(pr),
                 count(q -> q.skippable, pr), count(q -> q.skip, pr)))
    for q in pr
        say(@sprintf("P14_ROW   pid=%3d lev=%d len=%6d nread=%4d elread=%9d ratio=%8.3f nwhole=%3d skippable=%d skip=%d why=%s",
                     q.pid, q.level, q.len, q.nread, q.elread,
                     q.len == 0 ? 0.0 : q.elread / q.len, q.nwhole,
                     q.skippable, q.skip, join(String.(q.why), "|")))
    end
    say(@sprintf("P14_SUM   skippable_elems=%d nonskippable_elems=%d",
                 sum(q.len for q in pr if q.skippable; init = 0),
                 sum(q.len for q in pr if !q.skippable; init = 0)))
end
