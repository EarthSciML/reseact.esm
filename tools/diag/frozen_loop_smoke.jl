...
# No compiled-RHS seam here on purpose: the RHS in the loop body is the toy
# above, not an EarthSciAST build, which is what makes this gate cheap. The
# frozen grid is integrator code and does not care which emitter produced the
# right-hand side it composes. (It used to include `rx_native_patch.jl`, the
# traced emitter's broadcast lowering, which retired with that emitter --
# tools/diag/README-retired.md.)
