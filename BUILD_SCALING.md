...
> **Six of these probes were deleted on 2026-09-15** and the commands below are
> kept as the record of what produced each number, not as commands that still
> run. `build_res_window*`, `forcing_size_ladder`, `aktbl_ladder`, `axisb_ab`,
> `axisb_evaltime` and `build_prof_delta` each check a fresh build by calling the
> `:oop` build product on host vectors; that product is now the compiled IR
> EarthSciAST's direct StableHLO emitter lowers and raises
> `E_TREEWALK_OOP_NOT_EVALUABLE` when called, so each aborts at its first rung.
> `tools/diag/README-retired.md` records which measurement each one backs and why
> re-pointing the check would have destroyed the measurement it exists to make.
> `build_driver_decomp`, `decode_cost` and `build_scaling` never evaluate the
> product and run unchanged.
