...
### `run_reseact.jl` (native in-place) — that's all it needs.

Builds two in-place `f!(du,u,p,t)` closures and drives the CPU Lie-Trotter solve
(SSPRK43 transport → Rosenbrock23/BlockDiagonal chemistry). This is the reference
the Reactant runner is validated against.
...
### `EarthSciMLBase` — unbundle `BlockDiagonal`
